...
$ This script prints out joints labels.
...
for i = 1:length( newLinkName )
    ...
    if ( abs( thisPoint(1) - lastPoint(1) ) < 0.001 ) && ( abs( thisPoint(2) - lastPoint(2) ) < 0.001 ) && ( abs( thisPoint(3) - lastPoint(3) ) < 0.001 )
        ...
        counter = counter + 1;
        text( newX(i), newY(i)-0.01, newZ(i)-0.04*counter, newLinkName(i), 'FontSize', 18 );
        text( newX(i), newY(i)-0.03+0.05, newZ(i)-0.04*counter, num2str(i-1), 'FontSize', 14 );

    % If the new frame is not on top of the old one.
    else
        % Just put the label at the new frame.
        counter = 0;
        text( newX(i), newY(i)-0.01, newZ(i), newLinkName(i), 'FontSize', 18 );
        text( newX(i), newY(i)-0.03+0.05, newZ(i), num2str(i-1), 'FontSize', 14 );
    end
