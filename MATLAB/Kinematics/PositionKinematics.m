function[] = PositionKinematics( q )
    ...
    % Pelvis to LArm
    dl     = [0.1620,0,0.1406,0,-0.2450,0];
    thetal = [q(14),pi/2+q(15),q(16),pi/2,q(19),q(20)+pi];
    al     = [-0.0125,0,-0.2760,0.2256,0.11,-0.6058];
    alphal = [-pi/2,pi/2,0,-pi/2,pi/2,0];
    ...
    Tinter = iRz*iTz*iTx*iRx;

    numberOfJoints = length(dl);
    for i = 1:numberOfJoints
        PelvisLArm(i).step = DH([dl(i);thetal(i);al(i);alphal(i)]);
        if i == 1
            PelvisLArm(i).base = Tpelvis*Tinter*PelvisLArm(i).step;
        else
            PelvisLArm(i).base = PelvisLArm(i-1).base*PelvisLArm(i).step;  
        end
    end
    ...
    for i = 1:numberOfJoints
        ...
        if i == 1
            PelvisRArm(i).base = Tpelvis*Tinter*PelvisRArm(i).step;
        else
